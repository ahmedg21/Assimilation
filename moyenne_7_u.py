...
while current_date <= end_date:
    ...
    # Sélection des données PM2.5, PM01, PM10, température et humidité pour la journée en cours, en excluant les valeurs manquantes
    cur.execute("""
        SELECT event, PM25, PM01, PM10, temperature, humidite
        FROM envir
        WHERE event >= %s AND event < %s
            AND PM25 IS NOT NULL AND PM25 != ''
            AND PM01 IS NOT NULL AND PM01 != ''
            AND PM10 IS NOT NULL AND PM10 != ''
            AND temperature IS NOT NULL AND temperature != ''
            AND humidite IS NOT NULL AND humidite != ''
