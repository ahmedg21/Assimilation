...
with engine.connect() as conn, conn.begin():
    ...
    if result[0] == 0:
        ...
        conn.execute(insert_query, {'date': date_now, 'pm25': pm25_m, 'pm10': pm10_m, 'pm01': pm01_m, 'temperature': temperature_m, 'humidity': humidity_m})
        print('Nouvelle ligne insérée.')
    else:
        # Si la date existe, mettre à jour les valeurs
        update_query = text("UPDATE data_j SET pm25_m = :pm25, pm10_m = :pm10, pm01_m = :pm01, temperature_m = :temperature, humidity_m = :humidity WHERE DATE(event) = :date")
        conn.execute(update_query, {'date': date_now, 'pm25': pm25_m, 'pm10': pm10_m, 'pm01': pm01_m, 'temperature': temperature_m, 'humidity': humidity_m})
