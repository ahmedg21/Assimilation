...
import statistics
import time
import threading  # Import de la bibliothèque threading pour la tâche en parallèle

# Fonction qui affiche le compte à rebours
def countdown_timer():
    for i in range(delai_en_secondes, 0, -1):
        print(f"Temps restant : {i} secondes", end='\r')
        time.sleep(1)
    print("Le délai est écoulé!")

# Définir le délai en secondes (24 heures)
delai_en_secondes = 86400

# Créer un thread pour afficher le compte à rebours
