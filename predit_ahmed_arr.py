...
with engine.connect() as conn, conn.begin():
    for index, row in predictions_df.iterrows():
        ...
        if result[0] == 0:
            ...
            insert_query = text(f"INSERT INTO prediction_a (date, PM25, PM10) VALUES (:date, :pm25, :pm10)")
            conn.execute(insert_query, {'date': row['date'], 'pm25': row['PM2.5'], 'pm10': row['PM10']})
            print('bou bess')
        else:
            # Si la date existe, mettre à jour les valeurs
            update_query = text(f"UPDATE prediction_a SET PM25 = :pm25, PM10 = :pm10 WHERE DATE(date) = :date")
            conn.execute(update_query, {'date': row['date'], 'pm25': row['PM2.5'], 'pm10': row['PM10']})
            print('yessal nagnouko')

print("Les valeurs prédites ont été insérées ou mises à jour dans la table 'prediction'.")
