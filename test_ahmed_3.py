...
with engine.connect() as conn, conn.begin():
    for index, row in predictions_df.iterrows():
        ...
        result = conn.execute(query, date=date_part).fetchone()

        if result[0] == 0:
            # Si la date n'existe pas, insérer une nouvelle ligne
            insert_query = text(f"INSERT INTO prediction_a (date, PM2.5, PM10) VALUES (:date, :pm25, :pm10)")
            conn.execute(insert_query, date=date_part, pm25=row['PM2.5'], pm10=row['PM10'])
        else:
            # Si la date existe, mettre à jour les valeurs
            update_query = text(f"UPDATE prediction_a SET PM2.5 = :pm25, PM10 = :pm10 WHERE DATE(date) = :date")
            conn.execute(update_query, date=date_part, pm25=row['PM2.5'], pm10=row['PM10'])
