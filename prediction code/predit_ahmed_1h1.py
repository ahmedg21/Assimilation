...
forecasted_value_n2_pm25 = round(forecast_n2_pm25.predicted_mean.values[1], 2)  # Récupérez les valeurs prédites
...
forecasted_value_n2_pm10 = round(forecast_n2_pm10.predicted_mean.values[1], 2)  # Récupérez les valeurs prédites
...
forecasted_value_n2_pm25 = forecast_n2_pm25.predicted_mean.values[2]  # Récupérez les valeurs prédites
...
forecasted_value_n2_pm10 = forecast_n2_pm10.predicted_mean.values[2]  # Récupérez les valeurs prédites
...
forecasted_value_n3_pm25 = round(forecast_n3_pm25.predicted_mean.values[3], 2)   # Récupérez les valeurs prédites
...
forecasted_value_n3_pm10 = round(forecast_n3_pm10.predicted_mean.values[3], 2)  # Récupérez les valeurs prédites
...
predictions_n1_n2_n3_df = pd.DataFrame({
    'date': [date_n1_pm25.strftime("%Y-%m-%d %H:%M:%S"), date_n2_pm25.strftime("%Y-%m-%d %H:%M:%S"), date_n3_pm25.strftime("%Y-%m-%d %H:%M:%S")],
    'PM2.5': [forecasted_value_n1_pm25[0], forecasted_value_n2_pm25[0], forecasted_value_n3_pm25[0]],
    'PM10': [forecasted_value_n1_pm10[0], forecasted_value_n2_pm10[0], forecasted_value_n3_pm10[0]],
    'IQA_PM2.5': [iqa_pm25_n1[0], iqa_pm25_n2[0], iqa_pm25_n3[0]],
    'IQA_PM10': [iqa_pm10_n1[0], iqa_pm10_n2[0], iqa_pm10_n3[0]]
})

with engine.connect() as conn, conn.begin():
    for index, row in predictions_n1_n2_n3_df.iterrows():
        # Vérifier si la date existe déjà dans la table "prediction_n1_n2_n3"
...
print("Les valeurs prédites pour n+1, n+2 et n+3 ont été insérées ou mises à jour dans la table 'prediction_n1_n2_n3'.")
