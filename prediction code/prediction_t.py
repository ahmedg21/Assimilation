...
p_pm25, d_pm25, q_pm25 = 3, 0, 1  # Ordres ARIMA pour PM2.5
p_pm10, d_pm10, q_pm10 = 3, 0, 1  # Ordres ARIMA pour PM10
...
forecasted_value_n1_pm25 = round(forecast_n1_pm25.predicted_mean.values[0], 2)  # Arrondir à 2 chiffres après la virgule
iqa_pm25_n1 = int((forecasted_value_n1_pm25 / 75) * 100)  # Calcul de l'IQA pour PM2.5

# Effectuez la prédiction pour le jour suivant (n+1) pour PM10
forecast_n1_pm10 = model_fit_pm10.get_forecast(steps=1)
forecasted_value_n1_pm10 = round(forecast_n1_pm10.predicted_mean.values[0], 2)  # Arrondir à 2 chiffres après la virgule
iqa_pm10_n1 = int((forecasted_value_n1_pm10 / 150) * 100)  # Calcul de l'IQA pour PM10
...
forecasted_value_n2_pm25 = round(forecast_n2_pm25.predicted_mean.values[1], 2)  # Arrondir à 2 chiffres après la virgule
iqa_pm25_n2 = int((forecasted_value_n2_pm25 / 75) * 100)  # Calcul de l'IQA pour PM2.5

# Effectuez la prédiction pour le jour après-demain (n+2) pour PM10
forecast_n2_pm10 = model_fit_pm10.get_forecast(steps=2)
forecasted_value_n2_pm10 = round(forecast_n2_pm10.predicted_mean.values[1], 2)  # Arrondir à 2 chiffres après la virgule
iqa_pm10_n2 = int((forecasted_value_n2_pm10 / 150) * 100)  # Calcul de l'IQA pour PM10
...
forecasted_value_n3_pm25 = round(forecast_n3_pm25.predicted_mean.values[2], 2)  # Arrondir à 2 chiffres après la virgule
iqa_pm25_n3 = int((forecasted_value_n3_pm25 / 75) * 100)  # Calcul de l'IQA pour PM2.5

# Effectuez la prédiction pour le jour n+3 pour PM10
forecast_n3_pm10 = model_fit_pm10.get_forecast(steps=3)
forecasted_value_n3_pm10 = round(forecast_n3_pm10.predicted_mean.values[2], 2)  # Arrondir à 2 chiffres après la virgule
iqa_pm10_n3 = int((forecasted_value_n3_pm10 / 150) * 100)  # Calcul de l'IQA pour PM10

# Créez un dataframe pour stocker les valeurs prédites et l'IQA
predictions_df = pd.DataFrame({
    'date': [date_demain, date_apres_demain, date_n3_pm25],
    'PM2.5': [forecasted_value_n1_pm25, forecasted_value_n2_pm25, forecasted_value_n3_pm25],
    'PM10': [forecasted_value_n1_pm10, forecasted_value_n2_pm10, forecasted_value_n3_pm10],
    'IQA_PM2.5': [iqa_pm25_n1, iqa_pm25_n2, iqa_pm25_n3],
    'IQA_PM10': [iqa_pm10_n1, iqa_pm10_n2, iqa_pm10_n3]
})
...
with engine.connect() as conn, conn.begin():
    for index, row in predictions_df.iterrows():
        ...
        query = text(f"SELECT COUNT(*) FROM prediction_t WHERE DATE(date) = :date")
        result = conn.execute(query, {'date': row['date']}).fetchone()

        if result[0] == 0:
            # Si la date n'existe pas, insérer une nouvelle ligne
            insert_query = text(f"INSERT INTO prediction_t (date, pm25_p, pm10_p, iqapm25_p, iqapm10_p) VALUES (:date, :pm25, :pm10, :iqa_pm25, :iqa_pm10)")
            conn.execute(insert_query, {'date': row['date'], 'pm25': row['PM2.5'], 'pm10': row['PM10'], 'iqa_pm25': row['IQA_PM2.5'], 'iqa_pm10': row['IQA_P>
            #print('bou bess')
        else:
            # Si la date existe, mettre à jour les valeurs
            update_query = text(f"UPDATE prediction_t SET pm25_p = :pm25, pm10_p = :pm10, iqapm25_p = :iqa_pm25, iqapm10_p = :iqa_pm10 WHERE DATE(date) = :d>
            conn.execute(update_query, {'date': row['date'], 'pm25': row['PM2.5'], 'pm10': row['PM10'], 'iqa_pm25': row['IQA_PM2.5'], 'iqa_pm10': row['IQA_P>
