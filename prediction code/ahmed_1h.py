...
# Ajoutez ces prédictions pour 1 heure au dataframe principal (predictions_df)
predictions_df = pd.concat([predictions_1h_df, predictions_df], ignore_index=True)

# Créez un dataframe pour stocker les valeurs prédites pour les jours n+1, n+2 et n+3
...
with engine.connect() as conn, conn.begin():
    for index, row in predictions_df.iterrows():
        ...
        query = text(f"SELECT COUNT(*) FROM prediction_a WHERE DATE(date) = :date")
        result = conn.execute(query, {'date': row['date']}).fetchone()

        if result[0] == 0:
            # Si la date n'existe pas, insérer une nouvelle ligne
            insert_query = text(f"INSERT INTO prediction_a (date, PM25, PM10, IQA_PM25, IQA_PM10) VALUES (:date, :pm25, :pm10, :iqa_pm25, :iqa_pm10)")
            ...
            update_query = text(f"UPDATE prediction_a SET PM25 = :pm25, PM10 = :pm10, IQA_PM25 = :iqa_pm25, IQA_PM10 = :iqa_pm10 WHERE DATE(date) = :date")
...
# Utilisez SQLAlchemy pour insérer ou mettre à jour les données prédites pour n+1, n+2 et n+3 dans une autre table
with engine.connect() as conn, conn.begin():
    for index, row in predictions_n1_n2_n3_df.iterrows():
        # Vérifier si la date existe déjà dans la table "prediction_n1_n2_n3"
        query = text(f"SELECT COUNT(*) FROM prediction_n1_n2_n3 WHERE DATE(date) = :date")
        result = conn.execute(query, {'date': row['date']}).fetchone()

        if result[0] == 0:
            # Si la date n'existe pas, insérer une nouvelle ligne
            insert_query = text(f"INSERT INTO prediction_n1_n2_n3 (date, PM25, PM10, IQA_PM25, IQA_PM10) VALUES (:date, :pm25, :pm10, :iqa_pm25, :iqa_pm10)")
            conn.execute(insert_query, {'date': row['date'], 'pm25': row['PM2.5'], 'pm10': row['PM10'], 'iqa_pm25': row['IQA_PM2.5'], 'iqa_pm10': row['IQA_PM10']})
            print('bou bess')
        else:
            # Si la date existe, mettre à jour les valeurs
            update_query = text(f"UPDATE prediction_n1_n2_n3 SET PM25 = :pm25, PM10 = :pm10, IQA_PM25 = :iqa_pm25, IQA_PM10 = :iqa_pm10 WHERE DATE(date) = :date")
            conn.execute(update_query, {'date': row['date'], 'pm25': row['PM2.5'], 'pm10': row['PM10'], 'iqa_pm25': row['IQA_PM2.5'], 'iqa_pm10': row['IQA_PM10']})
            print('yessal nagnouko')
